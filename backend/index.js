...
app.post("/api/generate/", async (req, res) => {
  let token;
  token =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2NGZkNzUzZWUyYWY2OGYwYWE5ZDBhNTMiLCJpYXQiOjE2OTQzMzI4NjUsImV4cCI6MTY5NjkyNDg2NX0.FgREMSxXXoIewmbIb5FUPsvjn95EdZm2ysXcbw4A4xs";

  const { amount, service } = req.body;
  const { name, email, bic, idno, iban } = await decodeToken(token);
  const qrcode = await composeTransfer({
    name,
    email,
    bic,
    idno,
